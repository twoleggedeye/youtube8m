...
if __name__ == "__main__":
  ...
  flags.DEFINE_bool(
      ...
      "If set, then --train_data_path must be frame-level features. "
      "Otherwise, --train_data_path must be aggregated video-level "
...
def get_input_data_tensors_old(reader,
                           data_pattern,
                           batch_size=1000,
                           num_epochs=None,
                           num_readers=1):
...
def get_input_data_tensors(reader,
                           ...
                           **unused_args):
  logging.info("Using batch size of " + str(batch_size) + " for training.")
  dirs_list = [os.path.join(data_path, d) for d in os.listdir(data_path)]
  with tf.name_scope("train_input"):
    return reader.prepare_reader(dirs_list, batch_size=batch_size)
...
def build_graph(reader,
  ...
  Args:
    ...
    model: The core model (e.g. logistic or neural net). It should inherit
           from BaseModel.
    train_data_path: glob path to the training data files.
  ...
  # TODO: change to be compatible with numpy reader
